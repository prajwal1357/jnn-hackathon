...
class QFDetPreHead(AnchorHead):
    def __init__(self,
        ...
        self.use_small_object_loss = kwargs.pop('use_small_object_loss', False)
        self.small_object_weight_boost = kwargs.pop('small_object_weight_boost', 0.5)
        self.small_object_cls_boost = kwargs.pop('small_object_cls_boost', 0.0)
        self.small_object_centerness_boost = kwargs.pop('small_object_centerness_boost', 0.0)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, centerness, quality_pred, labels,
        ...
        if len(pos_inds) > 0 and getattr(self, 'use_small_object_loss', False) and getattr(self, 'small_object_cls_boost', 0.0) > 0:
            pos_bbox_targets_cls = bbox_targets[pos_inds]
            w_cls = (pos_bbox_targets_cls[:, 2] - pos_bbox_targets_cls[:, 0]).clamp(min=1e-6)
            h_cls = (pos_bbox_targets_cls[:, 3] - pos_bbox_targets_cls[:, 1]).clamp(min=1e-6)
            areas_cls = w_cls * h_cls
            small_thresh = 32.0 ** 2
            cls_boost = getattr(self, 'small_object_cls_boost', 0.0)
            scale_weights_cls = 1.0 + cls_boost * torch.clamp(1.0 - (areas_cls / small_thresh), min=0.0)
            label_weights = label_weights.clone()
            label_weights[pos_inds] = label_weights[pos_inds] * scale_weights_cls
        ...
        quality_targets = quality_targets.reshape(-1).detach().requires_grad_()
        ...
        if len(pos_inds) > 0:
            ...
            # Small-Object-Weighted Loss (Strategy B)
            if getattr(self, 'use_small_object_loss', False):
                w = (pos_bbox_targets[:, 2] - pos_bbox_targets[:, 0]).clamp(min=1e-6)
                h = (pos_bbox_targets[:, 3] - pos_bbox_targets[:, 1]).clamp(min=1e-6)
                areas = w * h
                small_thresh = 32.0 ** 2  # 1024 px^2
                weight_boost = getattr(self, 'small_object_weight_boost', 0.5)
                scale_weights = 1.0 + weight_boost * torch.clamp(1.0 - (areas / small_thresh), min=0.0)
                bbox_weights = centerness_targets * scale_weights

                centerness_boost = getattr(self, 'small_object_centerness_boost', 0.0)
                if centerness_boost > 0:
                    scale_weights_ctr = 1.0 + centerness_boost * torch.clamp(1.0 - (areas / small_thresh), min=0.0)
                    centerness_loss_weights = scale_weights_ctr
                else:
                    centerness_loss_weights = None
            else:
                bbox_weights = centerness_targets
                centerness_loss_weights = None
            ...
            loss_centerness = self.loss_centerness(
                pos_centerness,
                centerness_targets,
                weight=centerness_loss_weights,
