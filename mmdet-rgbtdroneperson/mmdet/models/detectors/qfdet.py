...
class ModalityGate(nn.Module):
    """Spatially-aware Modality Gate (Trust Meter) for RGB-Thermal Fusion."""
    ...
    def forward(self, rgb_feat, thermal_feat):
        ...
        weight = self.gate_conv(combined)
        fused = weight * rgb_feat + (1 - weight) * thermal_feat
        return fused, weight
...
class QFDet(SingleStageDetector):
    """Implementation of `QFDet` with optional ModalityGate."""
    ...
    def __init__(self,
        ...
        if self.use_modality_gate:
            self.modality_gate = ModalityGate(neck['out_channels'])
    ...
    def qce_fusion(self, x, quality_t, quality_v):
        ...
        for i in range(num_level):
            ...
            if getattr(self, 'use_modality_gate', False):
                gate_fused, weight_map = self.modality_gate(x_v, x_t)
                x_v = gate_fused
                if not hasattr(self, 'last_gate_weights'):
                    self.last_gate_weights = []
                self.last_gate_weights.append(weight_map)
